    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('erp.transaksi.update', $transaction->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="nasabah_id" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
                                ...
                                <option value="{{ $n->id_nasabah }}" {{ old('nasabah_id', $transaction->nasabah_id) == $n->id_nasabah ? 'selected' : '' }}>
                                    {{ $n->nama }} - {{ $n->no_ktp }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="product_type_id" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
                                ...
                                <option value="{{ $pt->id }}" {{ old('product_type_id', $transaction->product_type_id) == $pt->id ? 'selected' : '' }}>
                                    {{ $pt->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="amount" value="{{ old('amount', $transaction->amount) }}" required min="0" step="0.01" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
                            @error('amount')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="transaction_date" value="{{ old('transaction_date', $transaction->transaction_date->format('Y-m-d')) }}" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="notes" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">{{ old('notes', $transaction->notes) }}</textarea>
                            @error('notes')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
